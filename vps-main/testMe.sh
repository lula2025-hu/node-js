...
check_installations() {
    ...
    if ! command -v python3 &> /dev/null; then
        ...
        sudo apt-get update && sudo apt-get install -y python3 python3-pip git wget unzip
    fi
...
download_project() {
    if [ ! -d "$PROJECT_DIR" ]; then
        ...
        if command -v git &> /dev/null; then
            ...
            unzip -q project.zip && mv python-xray-argo-main "$PROJECT_DIR" && rm project.zip
        fi
...
if [ "$MODE" == "1" ]; then
    ...
    [ -z "$new_uuid" ] && new_uuid=$(generate_uuid) && echo -e "${GREEN}自动生成UUID: $new_uuid${NC}"
...
# 完整模式操作
    ...
    [ -n "$new_cfport" ] && update_file CFPORT "$new_cfport"
    ...
    [ -n "$new_argo_port" ] && update_file ARGO_PORT "$new_argo_port"
    ...
    if [[ "$adv_choice" =~ ^[Yy]$ ]]; then
        ...
        if [ -n "$new_domain" ]; then
            ...
            [ -n "$new_auth" ] && update_file ARGO_AUTH "$new_auth"
...
# 显示配置摘要（提前获取减少多次文件读取）
APP_UUID=$(grep "UUID = " app.py | head -1 | cut -d"'" -f2)
APP_NAME=$(grep "NAME = " app.py | head -1 | cut -d"'" -f2)
APP_PORT=$(grep "PORT = int" app.py | grep -o "or [0-9]*" | head -1 | cut -d" " -f2)
APP_CFIP=$(grep "CFIP = " app.py | cut -d"'" -f2)
echo -e "${YELLOW}=== 当前配置信息 ===${NC}"
echo -e "UUID: ${APP_UUID}"
echo -e "节点名称: ${APP_NAME}"
echo -e "服务端口: ${APP_PORT}"
echo -e "优选IP: ${APP_CFIP}"
echo -e "${YELLOW}========================${NC}"
echo
...
APP_PID=$(pgrep -f -n "python3 app.py")
if [ -z "$APP_PID" ]; then
    sleep 2
    APP_PID=$(pgrep -f "python3 app.py" | head -1)
    [ -z "$APP_PID" ] && { echo -e "${RED}服务启动失败！${NC}"; exit 1; }
fi
...
while [ $WAITED -lt $MAX_WAIT ]; do
    ...
    (( WAITED+=5 ))
    [ $((WAITED % 30)) -eq 0 ] && echo -e "${YELLOW}已等待 ${WAITED}s，继续等待...${NC}"
    sleep 5
done

[ -z "$NODE_INFO" ] && { echo -e "${RED}等待超时，未生成节点信息${NC}"; exit 1; }

# 提前获取配置数据，减少后续多次调用grep
SERVICE_PORT="$APP_PORT"
CURRENT_UUID="$APP_UUID"
